...
/**
 * Clase que representa al vendedor (viajante) que tiene que buscar el camino
 * minimo para recorrer todas las ciudades.
 * 
 */
public class Vendedor {
	/**
	 * matriz bidimensional que representa la ciudad
	 */
	int[][] ciudad;
	/**
	 * almacenamos las ciudades por cuya suma (coste del camino) sea la minima
	 */
	int solMejor[];
	/**
	 * alamacenamos el coste mejor
	 */
	double costeMejor;
	/**
	 * numero de ciudades
	...
	int cantDnodo;
	/**
	 * representa la ciudad reducida, es utilizada solo par la estumacion "muy
	 * ajustada"
	 */
	int[][] ciudadReducida;
	/**
	 * Definimos el tipo de estimacion, el mismo se hara desde el main
	 */
	private Estimacion tipo;
	...
	public void inciaVenta() {
		...
		PriorityQueue<Node> c = new PriorityQueue<Node>(10, comparator);
		int valEstimacion = 0;
		int minG = 0;
		...
		while ((!c.isEmpty()) && c.element().costeEstimado < costeMejor) {
			...
			for (int vertice = 1; vertice < n; vertice++) {
				...
				// si no a sido usado el verice por el cual vamos y el y si existe una arista
				// desde
				...
				if ((!x.usados[vertice]) && gvEstaArista(anterior, vertice)) {
					...
					x.ciudadReducida = copiarCiudad(y.ciudadReducida);
					if (x.k == n - 1) {// entramos si hemos recorrido desde el verice k al vertice n, pero hay que ver
										// si existe camino del vertice n al vertice 1, que en nuestro caso es el
										// 0(cero)
						if (gvEstaArista(x.sol[n - 1], 0) // verificamos si hay camino del vertice n al vertice 1
								&& ((x.coste + ciudad[x.sol[n - 1]][0]) < costeMejor)) { // y ademas si el coste que
																							// llevamos + el valor del
																							// camino antes
																							// visto mejora al coste de
																							// la mejor solucion
						...
						if (Estimacion.MUYAJUSTADA == tipo)
							...
							// x.coste(coste del camino actual) + minG (camino minimo de todo el grafo) * lo
	...
	private double costeEstimado2(Node nodoX, Node nodoY, int i, int j) {
		...
		// reducimos la matriz por fila, nos devolverá la suma de todoas las filas de
		// dicha reduccion,
		// en caso de no poder reducirse devolvera 0
		double b = reduceMatrizPorFila(nodoX.ciudadReducida);
		// reducimos la matriz por fila, nos devolverá la suma de todoas las columnas de
		// dicha reduccion,
		// en caso de no poder reducirse devolvera 0
